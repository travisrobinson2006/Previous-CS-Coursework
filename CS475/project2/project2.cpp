...
//            #pragma omp parallel for schedule(dynamic) reduction(+:fx,fy,fz)
	...
	double bodies_per_second = ((float)(NUMBODIES*NUMBODIES*NUMSTEPS)/(time1-time0)/(float)1000000.);
    fprintf(stdout,"Bodies computed per second: %10.2lf \n",bodies_per_second);
