...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Travis Robinson" };
const char *GLUITITLE   = { "User Interface Window for Travis Robinson" };
...
const char *CL_FILE_NAME   = { "particlesTravisRobinson.cl" };
...
/***Below was used to get MegaParticle Counts at a consistent time across changes in the number of particles--commented out for submission
	counter++;
	if(counter == 1000)
	{
		ElapsedTime = time1 - time0;
		fprintf(stdout,"%6.3f MegaParticles/Sec", (float)NUM_PARTICLES/ElapsedTime/1000000. );
	}*/
...
Display( )
{
	...
	{
		char str[128];
		sprintf( str, "%6.3f MegaParticles/Sec", (float)NUM_PARTICLES/ElapsedTime/1000000. );
...
InitCL( )
{
	...
	dPobj = clCreateFromGLBuffer( Context, 1, hPobj, &status );
	PrintCLError( status, "clCreateFromGLBuffer (1)" );

	dCobj = clCreateFromGLBuffer( Context, 2, hCobj, &status );
...
InitLists( )
{
	SphereList = glGenLists( 5 );
	glNewList( SphereList, GL_COMPILE );
		glPushMatrix( );
			glColor3f( Ranf( .3f, 1. ), Ranf( .3f, 1. ), Ranf( .3f, 1. ) );
...
ResetParticles( )
{
	...
	{
		points[i].x = Ranf( .5*XMIN, .5*XMAX );
		points[i].y = Ranf( .5*YMIN, .5*YMAX );
		points[i].z = Ranf( .5*ZMIN, .5*ZMAX );
		points[i].w = 1.;
	}
	...
	for( int i = 0; i < NUM_PARTICLES; i++ )
	{
		colors[i].r = 0.;//Ranf( .3f, 1. );
		colors[i].g = 0.;//Ranf( .3f, 1. );
		colors[i].b = 1.;//Ranf( .3f, 1. );
		colors[i].a = 1.;
	}
	glUnmapBuffer( GL_ARRAY_BUFFER );
