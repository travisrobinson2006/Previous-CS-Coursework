...
{
	for (int NUM_ELEMENTS = 1000; NUM_ELEMENTS < 8200000; NUM_ELEMENTS *= 2)
	{
		float result = 0.;
		#define	NUM_WORK_GROUPS		NUM_ELEMENTS/LOCAL_SIZE
	...
	size_t sizeReduction = NUM_WORK_GROUPS * sizeof(float);
	...
	cl_mem dD = clCreateBuffer( context, CL_MEM_WRITE_ONLY, sizeReduction, NULL, &status );
	if( status != CL_SUCCESS )
	...
	status = clEnqueueReadBuffer( cmdQueue, dD, CL_TRUE, 0, sizeReduction, hD, 0, NULL, NULL );
	if( status != CL_SUCCESS )
