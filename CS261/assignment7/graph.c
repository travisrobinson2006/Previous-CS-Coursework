...
** Date: 08/14/2015
** Description: implementation of BFS and DFS
** Input:main directing creation of graphs
** Output: DFS and BFS output 1 for paths, 0 else
*********************************************************************/
#include "graph.h"
...
//    return DFSRecursive(g,source,destination);

    clearVisited(g);
	struct cirListDeque* stk = malloc (sizeof (struct cirListDeque));
	initCirListDeque(stk);
	addFrontCirListDeque(stk,source);
	while (stk->size != 0)
    {
        Vertex* temp = frontCirListDeque(stk);
        removeFrontCirListDeque(stk);
        temp->isVisited = 1;
        if (temp == destination)
        {
            free(stk);
        ...
        {
            for (int index = 0; index < temp->numNeighbors; index++)
            {
                if (temp->neighbors[index]->isVisited != 1)
                {
                    addFrontCirListDeque(stk,temp->neighbors[index]);
                }
            }
        }
    }
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
	/* FIXME you will write this */
    clearVisited(g);
	struct cirListDeque* fifo = malloc (sizeof (struct cirListDeque));
	initCirListDeque(fifo);
	addFrontCirListDeque(fifo,source);
	while (fifo->size != 0)
    {
        Vertex* temp = frontCirListDeque(fifo);
        removeFrontCirListDeque(fifo);
        temp->isVisited = 1;
        if (temp == destination)
        {
            free(fifo);
        ...
        else
        {
            for (int index = 0; index < temp->numNeighbors; index++)
            {
                if (temp->neighbors[index]->isVisited != 1)
                {
                    addBackCirListDeque(fifo,temp->neighbors[index]);
                }
            }
        }
    }
